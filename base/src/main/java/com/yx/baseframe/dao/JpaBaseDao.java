...
import com.yx.baseframe.common.SmartDataSource;
import com.yx.baseframe.core.AbstractBaseDaoSupport;
import com.yx.baseframe.util.DialectUtils;
import com.yx.baseframe.util.SQLUtils;
...
@Repository("baseDao")
public class JpaBaseDao<T, PK extends Serializable> extends
	AbstractBaseDaoSupport<JpaEntityManager> {
	@Autowired
	private SmartDataSource smartDataSource;
	/** Log */
	protected static Logger logger = LoggerFactory
			.getLogger(JpaBaseDao.class);
	...
	public Query createNativeQueryWithIndexParam(final String sql,
			final Object... values) {
		return this.getEntityManager().createNativeQueryWithIndexParam(sql,
				values);
	}
	...
	public <X> SearchResult<X> findPageWithNameParamByNativeSQL(final int firstResult,
			final int maxResult, final String sql, final Map<String, ?> values) {

		SearchResult searResult = new SearchResult();

		List<Object[]> objectList = createNativeQueryWithNameParam(sql, values)
		...
		Object totalCount = createNativeQueryWithNameParam(countSql, values)
				.getSingleResult();
	...
	public Query createNativeQueryWithNameParam(final String sql,
		...
		return this.getEntityManager().createNativeQueryWithNameParam(sql,
				values);
	}
	...
	public void removeById(final Class<T> entityClass, final PK id) {
		this.getEntityManager().remove(
				this.getEntityManager().get(entityClass, id));
	...
	public List<T> get(final Class<T> entityClass, final Collection<PK> ids) {
		...
		return searchProcessor.search(this.getEntityManager()
				.getPersistEntityManager(), search);
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<T> getAll(final Class<T> entityClass, String orderByProperty,
			boolean isDesc) {
		Search search = new Search(entityClass);
		SearchUtil.addSort(search, orderByProperty, isDesc);
		return searchProcessor.search(this.getEntityManager()
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<T> findByProperty(final Class<T> entityClass,
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<T> findByPropertyAndOrder(final Class<T> entityClass,
			final String propertyName, final Object value,
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<T> findByPropertyAndOrderWithParams(final Class<T> entityClass,
			final String propertyName, final Collection<?> values,
			final String orderByProperty, boolean isDesc) {
		Assert.hasText(propertyName, "propertyName不能为空");
	...
	public T findUniqueByProperty(final Class<T> entityClass,
		...
		return (T) searchProcessor.searchUnique(this.getEntityManager()
				.getPersistEntityManager(), search);
	}
	...
	public <X> List<X> findWithIndexParam(final String jql,
			final Object... values) {
		return this.getEntityManager().createQueryWithIndexParam(jql, values)
				.getResultList();
	}
	...
	public <X> List<X> findWithNameParm(final String jql,
			final Map<String, ?> values) {
		return this.getEntityManager().createQueryWithNameParam(jql, values)
				.getResultList();
	}
	...
	public List<Object[]> findByNativeSQLWithIndexParam(final String sql,
		...
		return this.getEntityManager()
				.createNativeQueryWithIndexParam(sql, values).getResultList();
	}
	...
	public <X> X findUniqueWithIndexParam(final String jql,
			final Object... values) {
		try {
			return (X) this.getEntityManager()
					.createQueryWithIndexParam(jql, values).getSingleResult();
		} catch (javax.persistence.NoResultException e) {
			return null;
		}
	}
	...
	public <X> X findUniqueWithNameParam(final String jql,
			final Map<String, ?> values) {
		return (X) this.getEntityManager()
				.createQueryWithNameParam(jql, values).getSingleResult();
	}
	...
	public <X> SearchResult<X> findPageIndexParam(final int firstResult,
		...
		List objectList = this.getEntityManager()
				.createQueryWithIndexParam(jql, values)
				.setFirstResult(firstResult).setMaxResults(maxResult)
				.getResultList();
		// 查询记录总数
		String countJql = SQLUtils.buildCountSQL(jql);
		Long totalCount = (Long) this.getEntityManager()
				.createQueryWithIndexParam(countJql, values).getSingleResult();
	...
	public <X> SearchResult<X> findPageWithNameParam(final int firstResult,
		...
		List objectList = this.getEntityManager()
				.createQueryWithNameParam(jql, values)
				.setFirstResult(firstResult).setMaxResults(maxResult)
				.getResultList();
		// 查询记录总数
		String countJql = SQLUtils.buildCountSQL(jql);
		Long totalCount = (Long) this.getEntityManager()
				.createQueryWithNameParam(countJql, values).getSingleResult();
		...
		if (entityClass != null && !search.getSearchClass().equals(entityClass))
			throw new IllegalArgumentException(
					"Search class does not match expected type: "
							+ entityClass.getName());
		return this.searchProcessor.search(this.getEntityManager()
				.getPersistEntityManager(), entityClass, search);
	}
	...
	public <X> X searchUnique(Search search) {
		...
		if (search.getSearchClass() == null)
			throw new NullPointerException("Search class is null.");
		return (X) this.searchProcessor.searchUnique(this.getEntityManager()
				.getPersistEntityManager(), search);
	}
	...
	public int count(Search search) {
		...
		if (search.getSearchClass() == null)
			throw new NullPointerException("Search class is null.");
		return this.searchProcessor.count(this.getEntityManager()
				.getPersistEntityManager(), search);
	}
	...
	public Query createQueryWithIndexParam(final String queryJQL,
			final Object... values) {
		return this.getEntityManager().createQueryWithIndexParam(queryJQL,
				values);
	}
	...
	public Query createQueryWithNameParam(final String queryJQL,
			final Map<String, ?> values) {
		return this.getEntityManager().createQueryWithNameParam(queryJQL,
				values);
	}
}
